...
The committed application includes a structured proposal path through `@google/genai`. In the verified local build no provider credentials were configured, so the UI honestly labels the result **Study templates—not model-generated**. The configured source identifier is `gemini-3.7-flash`; live paid inference was not exercised for this build. When a permitted provider is configured, the server returns the provider and model metadata with the proposals rather than implying a model call that did not happen.
