...
describe('assessment private proxy session binding', () => {
  beforeEach(() => {
    proxy.mockReset();
    proxy.mockResolvedValue(new Response(JSON.stringify({ ok: true }), { status: 200, headers: { 'content-type': 'application/json' } }));
  });

  it('forwards and stores the same server-generated anonymous session identifier', async () => {
    const request = new NextRequest('https://sitepilot.example/api/assessment', {
      method: 'POST',
  ...
  it('ignores a browser-supplied session header and reuses the httpOnly cookie', async () => {
    const request = new NextRequest('https://sitepilot.example/api/assessment', {
      method: 'POST',
      headers: {
        origin: 'https://sitepilot.example', host: 'sitepilot.example', 'content-type': 'application/json',
        cookie: 'sitepilot_session=trusted-cookie-session', 'x-sitepilot-session': 'browser-controlled-session',
      },
      body: '{}',
    });
    const response = await POST(request);
